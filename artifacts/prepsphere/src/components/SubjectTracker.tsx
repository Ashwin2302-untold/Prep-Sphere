    ...
    <>
          ...
          <div className="flex-1 text-left">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                {totalNotes > 0 && (
                  <span className="flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] bg-violet-500/15 text-violet-300 border border-violet-500/25">
                ...
                <div className="space-y-1 max-h-72 overflow-y-auto pr-1 custom-scroll">
                  {filtered.map((chapter, _) => {
                    ...
                    return (
                      <div
                        key={realIdx}
                        className="flex items-center justify-between p-2.5 rounded-lg bg-white/5 hover:bg-white/8 transition-colors group"
                      >
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          <div className={`w-2 h-2 rounded-full flex-shrink-0 ${STATUS_CONFIG[chapter.status].dot}`} />
                          <span className="text-sm text-white/80 truncate">{chapter.name}</span>
                          {hasNote && (
                          ...
                          <button
                            ...
                            className={`p-1.5 rounded-lg border transition-all ${
                              hasNote
                                ...
                                : "bg-white/5 text-white/25 border-white/10 opacity-0 group-hover:opacity-100 hover:text-violet-300 hover:bg-violet-500/15 hover:border-violet-500/25"
                            }`}
                          >
      ...
      {activeChapter && (
        <NoteModal
          ...
          onSave={note => {
            if (noteChapterIdx !== null) onSaveNote(noteChapterIdx, note);
          }}
