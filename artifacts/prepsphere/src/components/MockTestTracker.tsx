    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-white">Mock Test Scores</h3>
        </div>
        <button
          data-testid="button-add-mock-test"
          onClick={() => setShowForm(!showForm)}
          className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-purple-500/20 text-purple-300 border border-purple-500/30 text-sm hover:bg-purple-500/30 transition-colors"
        >
      ...
      {showForm && (
        ...
        >
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-white/50 mb-1 block">Exam</label>
              <select
                data-testid="select-mock-exam"
                value={form.exam}
                onChange={e => setForm({ ...form, exam: e.target.value })}
                className="w-full bg-white/10 border border-white/10 text-white text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-purple-400/50"
              >
            ...
            <div>
              <label className="text-xs text-white/50 mb-1 block">Score</label>
              <input
            ...
            <div>
              <label className="text-xs text-white/50 mb-1 block">Total Marks</label>
              <input
          ...
          <button
            ...
            type="submit"
            className="w-full py-2 rounded-lg bg-purple-500/30 text-purple-200 border border-purple-500/40 hover:bg-purple-500/40 transition-colors text-sm font-medium"
          >
            Save Score
          </button>
        </motion.form>
      ...
      {recent.length === 0 ? (
        <p className="text-white/30 text-sm text-center py-6">No mock tests logged yet</p>
      ) : (
          ...
          {recent.map((t) => {
            ...
            return (
              <div key={t.id} className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5">
                <div>
                  <div className="text-sm font-medium text-white">{t.exam}</div>
                  <div className="text-xs text-white/40">{new Date(t.date).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</div>
                </div>
                <div className="text-right">
                  <div className={`text-lg font-bold font-mono ${color}`}>{t.score}<span className="text-white/30 text-sm">/{t.total}</span></div>
